...
![User Entity](/images/user-entity.png)
`Add a prop` 버튼을 클릭하여 필드를 추가합니다.

로그인에 사용할 아이디 필드를 추가해봅시다.
아래와 같이 정보를 입력합니다.

![Add Login ID](/images/add-login-id.png)
...
![Add Role Enum](/images/enum-user-role.png)

그리고 `role` 필드를 추가합니다.

![Add Role Field](/images/add-role.png)

`Type`은 `enum`을 선택하고, `Enum ID`는 위에서 생성한 `UserRole`로 지정합니다.

그 외에 비밀번호, 이름, 나이 필드를 추가하여 아래와 같이 유저 엔티티를 정의합니다.

![User Entity Full](/images/user-entity2.png)
...
![Prepared Migration Codes](/images/migration-user.png)
...
![Migration Applied](/images/migration-applied.png)

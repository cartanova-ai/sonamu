...
class UserModelClass extends BaseModelClass {
  ...
  async uploadAvatar(ctx: Context) {
    ...
    const url = await file.saveToDisk("public", `avatars/${ctx.user.id}.png`);
    return url;
  }
  ...
  async uploadDocument(ctx: Context) {
    const { bufferedFiles } = Sonamu.getContext();
    const file = bufferedFiles?.[0]; // 첫 번째 파일 사용
    const url = await file.saveToDisk("fs", `documents/${ctx.user.id}/${file.filename}`);
    return url;
  }
...
class FileModelClass extends BaseModelClass {
  ...
  async upload() {
    ...
    return uploadedFiles?.[0]?.url; // S3 URL (이미 스트리밍 완료)
  }
  ...
  async processTempFile() {
    ...
    await file.saveToDisk("fs", tempPath); // 로컬

    ...
    storage: {
      ...
      drivers: {
        s3: drivers.s3({
          ...
          bucket: "my-bucket",
          urlBuilder: {
            generateURL: (key) => `https://${cdnDomain}/${version}/${key}`,
          },
        }),
      },
    },
    ...
    storage: {
      ...
      drivers: {
        ...
        images: drivers.s3({
          ...
          urlBuilder: {
            generateURL: (key) => `https://images.cdn.example.com/${key}`,
          },
        }),
        ...
        videos: drivers.s3({
          ...
          urlBuilder: {
            generateURL: (key) => `https://videos.cdn.example.com/${key}`,
          },
        }),
      },
    },
...
5. **캐시 무효화**: URL 변경 시 CDN 캐시 고려
   ```typescript
   // 버전 파라미터 추가로 캐시 무효화
   generateURL: (key) => `/uploads/${key}?v=${Date.now()}`;
   ```
   </Warning>

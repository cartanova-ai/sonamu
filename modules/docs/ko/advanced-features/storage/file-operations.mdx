...
// bufferedFiles는 BufferedFile[] (optional 배열)
type Context = {
  // ...
...
```typescript
const file = bufferedFiles?.[0]; // 첫 번째 파일
```
...
<Tabs>
  <Tab title="filename" icon="file">
    **원본 파일명** ```typescript const {bufferedFiles} = Sonamu.getContext(); const file =
    bufferedFiles?.[0]; // 첫 번째 파일 사용 console.log(file.filename); // 'avatar.png' ```
  </Tab>

<Tab title="mimetype" icon="file-code">
  **MIME 타입** ```typescript console.log(file.mimetype); // 'image/png' // 'application/pdf' //
  'video/mp4' ```
</Tab>

<Tab title="size" icon="weight">
  **파일 크기** (바이트) ```typescript console.log(file.size); // 1024000 (1MB) console.log(`$
  {(file.size / 1024 / 1024).toFixed(2)}MB`); // '0.98MB' ```
</Tab>

<Tab title="extname" icon="file-zipper">
  **확장자** (점 제외) ```typescript console.log(file.extname); // 'png' // 'pdf' // 'mp4' // false
  (확장자 없음) ```
</Tab>

<Tab title="url" icon="link">
  **저장 후 URL** (unsigned) ```typescript await file.saveToDisk('fs', 'uploads/file.pdf');
  console.log(file.url); // '/uploads/file.pdf' (fs) //
  'https://bucket.s3.amazonaws.com/uploads/file.pdf' (s3) ```
</Tab>

  <Tab title="signedUrl" icon="lock">
    **저장 후 서명된 URL** (S3) ```typescript await file.saveToDisk('s3', 'uploads/file.pdf');
    console.log(file.signedUrl); // 'https://bucket.s3.amazonaws.com/uploads/file.pdf?X-Amz-...' ```
...
class PostModelClass extends BaseModelClass {
  ...
  async uploadImages(postId: number, ctx: Context) {
    ...
    // 이미지만 필터링
    const imageFiles = (bufferedFiles ?? []).filter((file) => file.mimetype.startsWith("image/"));
...
class FileModelClass extends BaseModelClass {
  ...
  async upload(visibility: "public" | "private", ctx: Context) {
    ...
    const diskName = visibility === "public" ? "public" : "fs";
...
<CardGroup cols={2}>
  ...
  <Card
    title="스토리지 드라이버"
    icon="hard-drive"
    href="/ko/advanced-features/storage/storage-drivers"
  >
    fs, S3 드라이버 설정
  </Card>

...
const ChatStreamEvents = z.object({
  chunk: z.object({
    text: z.string(),
  }),
  complete: z.object({
...
class ChatModelClass extends BaseModelClass {
  ...
  async streamChat(message: string, ctx: Context): Promise<void> {
    const sse = ctx.createSSE(ChatStreamEvents);
...
class ChatModelClass extends BaseModelClass {
  @api({ httpMethod: "POST" })
  async chat(message: string, conversationId: number | null, ctx: Context) {
    // 대화 이력 조회
    const history = conversationId ? await ConversationModel.findById(conversationId) : null;
...
<CardGroup cols={2}>
  ...
  <Card title="Agent 생성하기" icon="robot" href="/ko/advanced-features/ai-agents/creating-agents">

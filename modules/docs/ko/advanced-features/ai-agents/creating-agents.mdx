...
class HotelBookingAgentClass extends BaseAgentClass<{
  ...
  async searchHotels(input: { city: string; checkIn: string; checkOut: string; guests: number }) {
    // DB에서 호텔 검색
  ...
  async bookHotel(input: { hotelId: number; checkIn: string; checkOut: string; guests: number }) {
...
class DataAnalystAgentClass extends BaseAgentClass<{
  ...
  async calculateStats(input: { data: number[] }) {
    ...
    const variance =
      input.data.reduce((acc, val) => acc + Math.pow(val - mean, 2), 0) / input.data.length;
...
<CardGroup cols={2}>
  <Card title="Agent 설정" icon="gear" href="/ko/advanced-features/ai-agents/agent-configuration">
    Agent 기본 설정하기
  </Card>
  <Card title="AI SDK 사용하기" icon="code" href="/ko/advanced-features/ai-agents/using-ai-sdk">
    Vercel AI SDK 활용하기
  </Card>
</CardGroup>

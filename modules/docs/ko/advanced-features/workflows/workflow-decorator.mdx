...
Workflow는 백그라운드에서 실행되는 장기 실행 작업을 정의하는 시스템입니다. `workflow()` 함수를 사용하면 이메일 발송, 데이터 처리, 정기 작업 등을 안정적으로 실행할 수 있습니다.
...
<Tip>
  Workflow 이름을 생략하면 함수명을 snake_case로 변환하여 자동 설정합니다. 예: `sendWelcomeEmail` →
  `send_welcome_email`
</Tip>
...
export const processPayment = workflow(
  ...
  async ({ input, step, logger }) => {
    ...
    const orderId = await step
      .define({ name: "create_order" }, async () => {
...
<Warning>
  버전을 변경하면 기존 실행 중인 Workflow는 **코드에 이전 버전 로직이 있어야** 완료됩니다. 이전 버전
  코드를 삭제하기 전에 모든 실행이 완료되었는지 확인하세요.
</Warning>
...
export const dailyReport = workflow(
  ...
  async ({ input, step, logger }) => {
    ...
    const report = await step
      .define({ name: "generate_report" }, async () => {
...
<Tip>`input` 파라미터에 함수를 제공하면 **실행 시점의 데이터**를 동적으로 생성할 수 있습니다.</Tip>
...
export const sendBulkEmail = workflow(
  ...
  async ({ input, step, logger }) => {
    ...
    for (const user of users) {
      await step
        .define({ name: `send_email_${user.id}` }, async () => {
          await sendEmail({
            to: user.email,
...
export const processUserData = workflow(
  ...
  async ({ input, step, logger }) => {
    ...
    const rawData = await step
      .define({ name: "collect_data" }, async () => {
...
export const cleanupOldData = workflow(
  ...
  async ({ input, step, logger }) => {
    ...
    const deleted = await step
      .define({ name: "delete_old_records" }, async () => {
        return await knex("logs").where("created_at", "<", cutoffDate).delete();
      })
      .run();
...
<Tip>
  Workflow가 일시중지되면 현재 실행 중인 Step은 완료될 때까지 계속 실행됩니다. 다음 Step을 시작하기
  전에 Worker가 `paused` 상태를 확인하고 작업을 중단합니다.
</Tip>
...
<CardGroup cols={2}>
  ...
  <Card title="Worker 설정" icon="gear" href="/ko/advanced-features/workflows/worker-setup">

...
export const fetchWithRetry = workflow(
  ...
  async ({ input, step, logger }) => {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        const data = await step
          .define({ name: `fetch_attempt_${attempt}` }, async () => {
...
export const fetchWithBackoff = workflow(
  ...
  async ({ input, step, logger }) => {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        return await step
          .define({ name: `attempt_${attempt}` }, async () => {
...
```typescript
export const processOrder = workflow({ name: "process_order" }, async ({ input, step, logger }) => {
  let paymentId: string | null = null;
  let inventoryReserved = false;

  try {
    // Step 1: 결제
    paymentId = await step
      .define({ name: "charge_payment" }, async () => {
    ...
    // Step 2: 재고 확보
    await step
      .define({ name: "reserve_inventory" }, async () => {
    ...
    if (inventoryReserved) {
      await step
        .define({ name: "rollback_inventory" }, async () => {
    ...
    if (paymentId) {
      await step
        .define({ name: "rollback_payment" }, async () => {
...
```typescript
export const processData = workflow({ name: "process_data" }, async ({ input, step, logger }) => {
  try {
    await step
...
```typescript
export const sendEmail = workflow({ name: "send_email" }, async ({ input, step, logger }) => {
  const maxRetries = 3;

  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      await step
        .define({ name: `send_attempt_${attempt}` }, async () => {
...
export const callAPI = workflow({ name: "call_api" }, async ({ input, step, logger }) => {
  try {
    const data = await step
      .define({ name: "api_call" }, async () => {
        return await circuitBreaker.call(() => fetch(input.url).then((r) => r.json()));
      })
      .run();
...
```typescript
export const uploadFiles = workflow({ name: "upload_files" }, async ({ input, step, logger }) => {
  const results = [];
  ...
  for (let i = 0; i < input.files.length; i++) {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        await step
          .define({ name: `upload_${i}_attempt_${attempt}` }, async () => {

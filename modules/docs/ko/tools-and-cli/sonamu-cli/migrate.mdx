...
- 기본값은 현재 `NODE_ENV` preset
- `development`, `staging`, `production`, `test` 같은 명시적 preset
- 읽기 전용 preset은 제외
...
Sonamu.runScript(async () => {
  ...
  await migrator.runAction("rollback", ["development", "test"]);
...
// 재적용
await migrator.runAction("apply", ["development"]);
```
...
```bash
# 프로덕션 마이그레이션 전 백업
pg_dump -U postgres myapp_prod > backup_$(date +%Y%m%d).sql

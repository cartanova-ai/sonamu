...
| `--traces`         | `-t`        | 테스트에서 수집된 trace 정보를 출력 |
| `--status`         | `-s`        | DevRunner 상태를 확인하고 종료      |

<Tip>
  파일 경로는 여러 개를 지정할 수 있습니다: ```bash pnpm sonamu test
  src/application/user/user.test.ts src/application/post/post.test.ts ```
</Tip>

## Trace 출력

`--traces` (`-t`) 옵션을 사용하면 테스트 실행 중 `Naite.t()`로 수집된 trace 정보를 결과와 함께 출력합니다. 각 trace는 키, 소스 위치, 값을 포함합니다.
...
테스트 코드에서 `Naite.t()`를 사용하여 trace를 기록할 수 있습니다:
...
CLI에서 `--status` 플래그를 사용하거나 HTTP 엔드포인트에 직접 요청하여 DevRunner 상태를 확인할 수 있습니다:

```bash
# CLI로 확인
pnpm sonamu test --status

# 또는 HTTP 엔드포인트에 직접 요청
...
| `sseAvailable` | SSE(Server-Sent Events) 실시간 스트리밍이 사용 가능한지 여부 |
...
<CardGroup cols={2}>
  <Card title="테스트 구조" icon="vial" href="/ko/testing/writing-tests/test-structure">

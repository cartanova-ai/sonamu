...
```
DUMP...
  ✓ Schema dumped from development
...
2. **cone.note + LLM** — `--use-llm` 옵션이 켜져 있고, 해당 필드의 cone에 `note`가 지정된 경우 LLM이 `note` 내용을 기반으로 현실적인 값을 생성합니다. LLM 호출이 실패하면 다음 단계로 넘어갑니다.
...
```bash title=".env.test"
SONAMU_DB_FIXTURE_HOST=fixture-db.example.com
SONAMU_DB_FIXTURE_PORT=5432
SONAMU_DB_FIXTURE_USER=postgres
SONAMU_DB_FIXTURE_PASSWORD=your-fixture-password
SONAMU_DB_FIXTURE_NAME=myapp_fixture
```

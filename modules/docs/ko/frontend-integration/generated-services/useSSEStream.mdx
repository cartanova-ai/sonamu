...
<Warning>
`useSSEStream`은 `services.generated.ts`가 아니라 `@sonamu-kit/react-components`에서 import합니다.
</Warning>
...
SSE 엔드포인트는 `@api` 데코레이터의 `sse: true` 옵션으로 만듭니다.
...
**해결**: 백엔드에서 `yield { event, data: JSON객체 }` 형식을 사용하세요.

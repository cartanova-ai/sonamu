...
<Note>
  `entry-server.generated.tsx`는 입력에 의존하지 않는 정적 부트스트랩 자산입니다. 첫 sync 시 1회만
  생성되며(`IfNotExists`), 이후에는 추적 사이클 밖에 있어 Model 변경에 따라 재생성되지 않습니다.
  필요 시 직접 편집해도 됩니다.
</Note>
...
`.types.ts` 파일을 저장하면 스키마 파일들이 재생성되고, 타겟에 복사됩니다.

<Note>
  `.types.ts` 파일은 Single Source of Truth로 취급됩니다. 변경 시 Entity 변경과 동일하게 스키마
  파일들이 재생성됩니다.
</Note>
...
Types 파일에서 정의한 Zod 스키마들이 이 파일에 반영됩니다.
...
Types 변경에 따른 타입 정합성을 유지하기 위해 함께 재생성됩니다.

</Step>

<Step title="3. 타겟 복사">
수정된 Types 파일과 재생성된 파일들이 타겟 프로젝트에 복사
...
| 변경 파일                | {entity}.types.ts | sonamu.generated.ts | sonamu.generated.sso.ts | services.generated.ts | sonamu.generated.http | queries.generated.ts | .sonamu.env | sd.generated.ts |
| ------------------------ | ----------------- | ------------------- | ----------------------- | --------------------- | --------------------- | -------------------- | ----------- | --------------- |
| **{entity}.entity.json** | ✅ (첫 생성)      | ✅                  | ✅                      | -                     | -                     | -                    | -           | -               |
| **{entity}.model.ts**    | -                 | -                   | -                       | ✅                    | ✅                    | ✅                   | -           | -               |
| **{entity}.types.ts**    | _복사_            | ✅                  | ✅                      | -                     | -                     | -                    | -           | -               |
| **sonamu.config.ts**     | -                 | -                   | -                       | -                     | -                     | -                    | ✅          | -               |
| **i18n/{locale}.ts**     | -                 | -                   | -                       | -                     | -                     | -                    | -           | ✅              |
...
<Info>
  `entry-server.generated.tsx`는 부트스트랩 자산이라 위 매트릭스에 포함되지 않습니다. 첫 sync에서만
  `IfNotExists`로 만들어지고, 이후 변경 추적 사이클 밖에 있어요.
</Info>
...
`--force`는 기존 lock을 무시하고 처음부터 모든 추적 자산을 다시 정합 상태로 맞춥니다. git
post-merge hook이나 CI에서 매 pull 후 자동 실행할 수 있도록 노출되어 있습니다.
...
<AccordionGroup>
  <Accordion title="필요한 파일만 변경">
    Model API 변경이 아니라면 Entity나 Types 수정만으로 충분

    - Types/Entity 변경 → 스키마 재생성
    - Model 변경 → 서비스 및 HTTP 파일 재생성

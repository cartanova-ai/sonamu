...
<CardGroup cols={2}>
  <Card title="생성 파일은 읽기 전용" icon="lock">
    `*.generated.*` 파일 절대 수정 금지 자동 재생성으로 변경 손실
  </Card>
  <Card title="확장으로 커스터마이징" icon="puzzle-piece">
    별도 파일에서 래핑 또는 확장 안전하고 유지보수 쉬움
  </Card>
  <Card title="소스에서 제어" icon="sliders">
    Entity, Model, Types에서 조정 재생성 시에도 유지됨
  </Card>
  <Card title="Template 활용" icon="file-code">
    커스텀 템플릿으로 생성 제어 프로젝트 요구사항 반영
  </Card>
</CardGroup>
...
export const UserSaveParams = z
  .object({
    email: z
      ...
      .refine((email) => !email.endsWith("@temp.com"), "임시 이메일은 사용할 수 없습니다"),
    ...
    age: z.number().int().min(18, "18세 이상만 가입 가능").max(100),
  })

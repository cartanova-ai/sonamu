...
Syncer가 sonamu.lock에 추적하는 파일 패턴입니다. 자산 본성에 따라 **입력**과 **출력**으로 나누고,
출력은 위치(api / target에 복사)별로 다시 분리합니다.
...
| **frame**      | `src/application/**/*.frame.ts`     | Entity 없는 API 프레임 |
...
| 파일 타입         | 패턴                                          | 용도                                |
| ----------------- | --------------------------------------------- | ----------------------------------- |
| **generated**     | `src/application/**/*.generated.{ts,tsx,sso.ts}` | sonamu.generated.\*, queries.generated 등 |
| **httpGenerated** | `src/application/**/*.generated.http`         | REST Client용 HTTP 테스트 파일       |
...
| **functionsCopied**   | `src/services/**/*.functions.ts`                          | Functions 사본                        |
| **i18nCopied**        | `src/i18n/**/!(sd.generated).ts`                          | i18n 소스 사본                        |

<Info>
  **파일 패턴**: `syncer/file-patterns.ts`에서 단일 진실 원천(SSOT)으로 관리됩니다. 추적 자산은
  checksum으로 변경 감지하며, `FileType`은 패턴 그룹 함수의 키에서 자동 추론됩니다.
</Info>

<Note>
  **추적 밖 부트스트랩 자산** — `sonamu.shared.ts`(IfNotExists로 1회 생성, 사용자 커스터마이즈
  영역)와 `entry-server.generated.tsx`(입력 의존 없는 정적 코드, IfNotExists로 1회 생성)는 sync()의
  부트스트랩 phase에서 한 번 보장된 뒤 추적 사이클 밖에 둡니다. sonamu.lock에 기록되지 않습니다.
</Note>
...
```bash
# 변경된 파일만 동기화 (sonamu.lock 기준)
pnpm sonamu sync

# 강제 풀-싱크 (sonamu.lock 제거 후 처음부터 재싱크)
pnpm sonamu sync --force
```

`--force`는 git post-merge hook이나 CI에서 매 pull 후 자동 실행할 수 있도록 노출되어 있습니다.
dev 서버 실행 중에는 `f` 핫키로 같은 효과(lock 제거 + 재시작)를 낼 수 있습니다.
...
// dev watcher가 모은 batch를 한 번에 처리 (HMR + sync 영역 분리)
...
<AccordionGroup>
  <Accordion title="파일이 생성되지 않음">
    **확인 사항**: 1. 개발 서버가 실행 중인지 확인 2. 파일이 감시 대상 패턴에 포함되는지 확인 3.
    강제 재동기화 시도 ```bash pnpm sonamu sync --force ```
  </Accordion>

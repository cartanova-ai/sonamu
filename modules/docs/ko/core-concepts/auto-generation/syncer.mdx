...
<Info>
**파일 패턴**: `file-patterns.ts`에서 관리되며, checksum으로 변경 감지
</Info>
...
<Info>
SSR 설정 변경은 체크섬 기반 동기화 대상이 아닙니다. Watcher가 직접 감지하여 별도의 경로로 처리합니다.
</Info>
...
<AccordionGroup>
  <Accordion title="파일이 생성되지 않음">
    **확인 사항**:
    1. 개발 서버가 실행 중인지 확인
    2. 파일이 감시 대상 패턴에 포함되는지 확인
    3. Checksum 파일 삭제 후 재시도
  ...
  <Accordion title="타겟에 파일이 복사되지 않음">
    **확인 사항**:
    1. `sonamu.config.ts`의 `sync.targets` 확인
    2. 타겟 디렉토리 존재 확인
    3. 타겟 디렉토리에 `src/services/` 디렉토리 생성
  ...
  <Accordion title="import 에러 발생">
    **원인**: `sonamu` import가 `./sonamu.shared`로 변환되지 않음

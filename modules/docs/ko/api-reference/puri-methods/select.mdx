...
const users = await puri.table("users").select({
  id: "users.id",
  display_label: Puri.concatExpressions("Sonamu", " ", "User"),
  email_upper: Puri.upper("users.email"),
  email_lower: Puri.lower("users.email"),
});
```

`Puri.concatExpressions()`는 바인딩된 리터럴 매개변수에 PostgreSQL `text` 타입 캐스트를
추가하므로 위 예시처럼 리터럴만 사용한 표현식도 실행할 수 있습니다. 기존 `Puri.concat()`은
리터럴을 타입이 지정되지 않은 매개변수로 바인딩하며 컬럼처럼 보이는 문자열도 컬럼으로 해석하지
않습니다. 따라서 리터럴만 사용한 독립적인 `Puri.concat()` 표현식은 PostgreSQL에서 실행할 수
없습니다. 문자열 SQL 표현식과 리터럴 텍스트를 조합하려면 `Puri.concatExpressions()`를 사용하세요.
...
일반 컬럼이나 null이 아닌 문자열 표현식을 전달하면 유사도 함수는 `SqlExpression<"number">`를
반환합니다. 이 표현식을 `rawNumber`와 조합하여 컬럼별 가중치 점수를 계산할 수 있습니다.
...
<Info>
결과는 입력 표현식의 null 가능성을 유지합니다. `jsonText()`는 nullable 문자열 표현식을 생성하며, 이를 유사도 함수에 전달하면
`SqlExpression<"number", true>`를 반환하고 선택 결과는 `number | null`입니다. 일반 컬럼이나 null이
아닌 표현식을 전달하면 `SqlExpression<"number">`를 반환하고 선택 결과는 `number`입니다. `pg_trgm`
확장이 설치되어 있어야 합니다.
</Info>
  ...
  <Tab title="사용자 + 게시물" icon="user-pen">
    ...
    const users = await puri.table("users")
      ...
      .select({
        ...
        // 타입이 지정된 문자열 표현식으로 계산한 필드
        full_info: Puri.concatExpressions(
          Puri.upper("users.name"),
          " (",
          Puri.lower("users.email"),
          ")"
        )
      });

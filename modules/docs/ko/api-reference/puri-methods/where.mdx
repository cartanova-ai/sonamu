...
- `like` - 패턴 매칭
- `not like` - 패턴 불일치
- `ilike` - 대소문자 무시 패턴 매칭 (PostgreSQL)
- `not ilike` - 대소문자 무시 패턴 불일치 (PostgreSQL)
...
### ILIKE (대소문자 무시, PostgreSQL)

PostgreSQL에서 대소문자를 구분하지 않는 패턴 매칭을 수행합니다.
...
// WHERE users.email NOT ILIKE '%SPAM%'
// 'spam', 'Spam', 'SPAM' 모두 제외
```

<Info>
`ilike`와 `not ilike`는 PostgreSQL 전용 연산자입니다. MySQL에서는 `LOWER()` 함수와 `like`를 조합하여 유사한 효과를 얻을 수 있습니다.
</Info>
...
PostgreSQL의 `pg_trgm` 확장을 사용한 퍼지 문자열 매칭입니다. trigram 기반으로 오타에 강하고, generated column과 GIN 인덱스를 활용하면 다중 컬럼 검색에도 효과적입니다.

```typescript
// 기본 사용법 (word similarity, <% 연산자)
await puri.table("items")
  .whereFuzzy("items.search_text", "typscript");
...
// similarity (% 연산자)
await puri.table("items")
  .whereFuzzy("items.search_text", "typscript", { operator: "%" });

// WHERE items.search_text::text % 'typscript'::text

// strict word similarity (<<% 연산자)
await puri.table("items")
  .whereFuzzy("items.search_text", "typscript", { operator: "<<%" });
...
| 연산자 | 의미 | SQL | 설명 |
|--------|------|-----|------|
| `<%` (기본) | word similarity | `'query'::text <% column::text` | 쿼리가 컬럼의 부분 문자열과 유사한지 확인 |
| `%` | similarity | `column::text % 'query'::text` | 전체 문자열 간 유사도 비교 |
| `<<%` | strict word similarity | `'query'::text <<% column::text` | word similarity보다 더 엄격한 부분 문자열 매칭 |

<Info>
`whereFuzzy`를 사용하려면 `pg_trgm` 확장이 설치되어 있어야 합니다. `CREATE EXTENSION IF NOT EXISTS pg_trgm`으로 설치할 수 있습니다. GIN 인덱스(`gin_trgm_ops`)를 생성하면 성능이 크게 향상됩니다.
</Info>
...
<Info>
PostgreSQL에서 `ilike`는 대소문자 무시 검색에 편리하지만, 대용량 데이터에서는 성능에 영향을 줄 수 있습니다.
이런 경우 `citext` 타입이나 `pg_trgm` 확장의 인덱스 사용을 고려하세요.
</Info>

...
<Info>
  `ilike`와 `not ilike`는 PostgreSQL 전용 연산자입니다. MySQL에서는 `LOWER()` 함수와 `like`를
  조합하여 유사한 효과를 얻을 수 있습니다.
</Info>
...
<Info>`whereMatch`를 사용하려면 해당 컬럼에 FULLTEXT 인덱스가 있어야 합니다.</Info>
...
<Info>PGroonga 검색은 사용할 인덱스와 **동일한 컬럼 구성**으로 검색해야 인덱스가 사용됩니다.</Info>
...
<Info>
  `whereFuzzy`를 사용하려면 `pg_trgm` 확장이 설치되어 있어야 합니다. `CREATE EXTENSION IF NOT EXISTS
  pg_trgm`으로 설치할 수 있습니다. GIN 인덱스(`gin_trgm_ops`)를 생성하면 성능이 크게 향상됩니다.
</Info>
...
<Warning>SQL 인젝션을 방지하기 위해 항상 파라미터 바인딩을 사용하세요.</Warning>
...
<Info>
  최상위 레벨에는 `orWhere`가 없습니다. OR 조건은 `whereGroup` 또는 `orWhereGroup` 내에서
  사용하세요.
</Info>
...
// ❌ 타입 에러: 잘못된 타입
await puri.table("users").where("users.age", "25"); // string (age는 number)
```
...
```typescript
// ✅ 좋음: 인덱스 컬럼 사용
await puri.table("users").where("users.email", "user@example.com"); // email에 인덱스 필요
...
<Info>
  PostgreSQL에서 `ilike`는 대소문자 무시 검색에 편리하지만, 대용량 데이터에서는 성능에 영향을 줄 수
  있습니다. 이런 경우 `citext` 타입이나 `pg_trgm` 확장의 인덱스 사용을 고려하세요.
</Info>
...
const ids = [];
await puri.table("users").whereIn("users.id", ids); // 아무것도 반환하지 않음

...
<Info>
`sonamuFilter`는 Entity의 props를 기반으로 자동 생성됩니다. 관계(relations) 및 가상 필드(virtual fields)는 필터링 대상에서 제외됩니다.
</Info>
...
Entity의 props에 정의된 필드만 필터링할 수 있습니다. 관계 필드나 가상 필드는 필터링 대상에서 제외됩니다.

```typescript
// ❌ 에러: 필드 'employee'는 필터링할 수 없습니다
await ProjectModel.findMany("A", {
  sonamuFilter: { employee: { eq: 1 } }  // 관계 필드
});

...
type UserListParams = {
  ...
  // 의미 검색 (벡터 검색)
  semanticQuery?: Record<string, unknown>;  // 의미 검색 파라미터
}
```
...
```typescript
// semanticQuery가 있으면 각 row에 similarity 필드가 추가됩니다.
...
`semanticQuery`를 전달하면 각 row에 `similarity` 필드(0~1 사이의 유사도 점수)가 자동으로 추가됩니다.

```typescript
// 일반 조회
const { rows } = await PostModel.findMany("A", { num: 10 });
// rows[0].id, rows[0].title  (similarity 없음)
...
// rows[0].id, rows[0].title, rows[0].similarity  (similarity 포함)
```
...
<CardGroup cols={2}>
  <Card 
    ...
    href="/ko/api-reference/base-model/find-by-id"
  >
    ID로 단일 레코드 조회
  </Card>

...
test("중복 사용자명 에러", async () => {
  ...
  // 중복 사용자명으로 생성 시도
  await expect(userModel.create({ username: "john" })).rejects.toThrow(
    "이미 존재하는 사용자명입니다",
  );
...
자세한 내용은 [API Mocking](/ko/testing/mocking/api-mocking)을 참고하세요.
...
4. **병렬 실행 제한**: 트랜잭션 격리를 위해 테스트는 순차 실행이 권장됩니다.
   </Warning>

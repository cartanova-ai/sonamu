...
<Info>**Expand-Contract 장점**: - 무중단 배포 가능 - 언제든지 롤백 가능 - 데이터 손실 없음</Info>
...
<Warning>
  **CONCURRENTLY 주의사항**: - 트랜잭션 외부에서 실행 - 시간이 더 오래 걸림 - 실패 시 INVALID 인덱스
  남음
</Warning>
...
<Info>
  **NOT VALID + VALIDATE 장점**: - 기존 데이터 검증 스킵 (빠름) - 새 데이터만 즉시 검증 - 나중에
  백그라운드로 전체 검증
</Info>

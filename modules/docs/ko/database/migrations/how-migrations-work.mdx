...
<Info>
  Migration 파일명: `{타임스탬프}_{작업}_{테이블명}.ts` - 타임스탬프: `YYYYMMDDHHmmss` 형식 - 작업:
  `create`, `alter`, `foreign`, `drop` 등 - 테이블명: 언더스코어 구분
</Info>
...
`where`는 PostgreSQL partial index를 만들기 위한 raw SQL predicate입니다. Sonamu는 DB
introspection으로 읽은 predicate와 entity.json의 predicate를 비교하여 같은 조건이면 추가 migration을
생성하지 않습니다.

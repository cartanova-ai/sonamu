...
<Info>`ubUpsert`는 등록된 모든 레코드를 **단일 INSERT 쿼리**로 처리하여 성능을 최적화합니다.</Info>
...
async function seedDatabase() {
  await db.transaction(async (trx) => {
    ...
    const companyRefs = companies.map((name) => trx.ubRegister("companies", { name }));
...
```typescript
async function reorganizeTeams(projectId: number, newMemberIds: number[]) {
  await db.transaction(async (trx) => {
    // 1. 기존 멤버 제거
...
<CardGroup cols={2}>
  <Card
    ...
    href="/ko/database/upsert-builder/advanced-patterns"
  >
    복잡한 데이터 구조
  </Card>

...
const results = await db.table("messages").select({
  greeting: Puri.concatExpressions("안녕하세요", ", ", "Sonamu"),
});
```
...
리터럴을 타입이 지정되지 않은 매개변수로 바인딩하며, `"users.name"` 같은 인자를 컬럼으로
해석하지 않습니다. 따라서 리터럴만 사용한 독립적인 `Puri.concat()` 표현식은 PostgreSQL에서
실행할 수 없습니다. 문자열 SQL 표현식과 리터럴 텍스트를 조합하려면 [JSON 필드
추출](#json-필드-추출)의 `Puri.concatExpressions()`와 `Puri.coalesce()`를 사용하세요.
...
const results = await query.select({
  ...
  summary: Puri.concatExpressions(
    Puri.coalesce(title, "제목 없음"),
    " — ",
    Puri.coalesce(description, "설명 없음"),
  ),
});
```

`jsonText()`는 생성된 JSON 컬럼과 하나 이상의 경로 조각을 받으며 쿼리를 변경하지 않고 문자열 SQL
표현식을 반환합니다. 경로가 없거나 해당 JSON 값이 `null`이면 PostgreSQL은 SQL `NULL`을 반환합니다.
따라서 `jsonText()`를 직접 선택하면 `string | null`로 추론됩니다. `Puri.coalesce()`로 감싸면 위
`summary`처럼 null이 아닌 `string`으로 추론됩니다. 경로 조각과 리터럴 텍스트, 대체값은 값으로
바인딩하며, `Puri.concatExpressions()`에는 문자열 리터럴과 문자열 SQL 표현식만 전달할 수 있습니다.
...
`whereJsonKeyExists()`는 PostgreSQL JSONB의 `?` 연산자를 사용하고 키를 값으로 바인딩합니다. 최상위
키가 있는지 확인하므로 JSON 값이 `null`인 키도 존재하는 것으로 처리합니다. AND/OR 우선순위를
명확히 유지하려면 그룹 안에서 `whereJsonKeyExists()`와 `orWhereJsonKeyExists()`를 사용하세요.

...
<Warning>
  **트랜잭션 안에 포함하지 말아야 할 것**: - 외부 API 호출 - 파일 I/O - 이메일/SMS 발송 - 복잡한
  계산 - 느린 쿼리
</Warning>
...
class OrderModelClass extends BaseModelClass {
  ...
  async createOrderWithPayment(
    ...
    // 1. 재고 확인 및 차감 (InventoryModel 호출)
    //    @transactional이 있으므로 같은 트랜잭션 컨텍스트 공유
...
class PaymentModelClass extends BaseModelClass {
  ...
  async processPayment(params: ProcessPaymentParams): Promise<number> {
    ...
    // 외부 결제 API 호출 (트랜잭션 밖에서 해야 하지만 여기선 예시)
...
- `createOrderWithPayment`가 최상위 트랜잭션 시작
- `InventoryModel.reserveStock`, `PaymentModel.processPayment`는 같은 트랜잭션 컨텍스트 공유
  ...
  async createOrderWithOptionalDiscount(
    ...
    if (couponCode) {
      try {
        ...
        this.logger.info("쿠폰 적용 성공", { orderId, couponCode });
      } catch (error) {
        // 쿠폰 적용 실패 시 이 부분만 롤백
        await wdb.raw("ROLLBACK TO SAVEPOINT coupon_apply");
        this.logger.warn("쿠폰 적용 실패, 주문은 유지", { orderId, error });
      }
    }
...
<Warning>
  **Savepoint 주의사항**: - Savepoint는 수동으로 생성/관리해야 합니다 (자동 생성 안 됨) - 중첩
  레벨이 깊어질수록 복잡도 증가 - 대부분의 경우 전체 롤백이 더 안전하고 단순합니다
</Warning>
...
class ProductModelClass extends BaseModelClass {
  ...
  async purchaseProduct(productId: number, quantity: number, userId: number): Promise<number> {
    ...
    const product = await wdb
      .table("products")
      .where("id", productId)
      .forUpdate() // ← 다른 트랜잭션이 이 행을 수정하지 못하도록 락
      .first();
...
| 방식                         | 사용 시점                 | 장점                    | 단점                        |
| ---------------------------- | ------------------------- | ----------------------- | --------------------------- |
| **비관적 락** (`FOR UPDATE`) | 충돌이 자주 발생하는 경우 | 데이터 일관성 보장 확실 | 대기 시간 증가, 처리량 감소 |
| **낙관적 락** (version 컬럼) | 충돌이 드문 경우          | 높은 처리량             | 충돌 시 재시도 필요         |
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (this.isDeadlockError(error)) {
          ...
          this.logger.warn(`데드락 발생, 재시도 ${attempt}/${maxRetries}`, {
            userId,
            error,
          });
        ...
        // 데드락이 아닌 다른 에러는 즉시 throw
        throw error;
      }
    }

    // 최대 재시도 횟수 초과
    throw new Error(`주문 생성 실패: 최대 재시도 횟수 초과 (${maxRetries}회)`, {
      cause: lastError,
    });
  }
...
class PaymentModelClass extends BaseModelClass {
  async processOrderPayment(orderId: number, paymentInfo: PaymentInfo): Promise<PaymentResult> {
    ...
    try {
      ...
      throw new PaymentError("결제 처리 실패", { cause: error });
    }

    // 3. 결제 성공 시 DB 업데이트 (트랜잭션)
    try {
      return await this.recordPaymentSuccess(orderId, paymentResult);
    } catch (error) {
      // DB 업데이트 실패 시 결제 취소 시도
...
<Info>
  **분산 트랜잭션 Best Practice**: - 외부 시스템과 통신할 때는 Saga 패턴 사용 - 각 단계마다 실패 시
  보상 작업 정의 - 멱등성(Idempotency) 보장으로 재시도 안전성 확보
</Info>

...
class PostModel extends BaseModelClass {
  ...
  async create(params: { title: string; content: string }): Promise<{ postId: number }> {
    const context = Sonamu.getContext();
...
class UserModel extends BaseModelClass {
  ...
  async getAllUsers(params: { page?: number; pageSize?: number }): Promise<{
    users: User[];
    total: number;
  }> {
    ...
    const [{ count }] = await rdb.table("users").count("* as count");
...
class FileModel extends BaseModelClass {
  ...
  async download(params: { fileId: number; token: string }): Promise<{
    url: string;
  }> {
...
<Info>
  `query` Guard의 구체적인 구현은 프로젝트마다 다를 수 있습니다. API 키, 토큰, 서명 등 다양한
  방식으로 확장 가능합니다.
</Info>
...
<Warning>
  **주의**: 복잡한 비즈니스 로직이나 데이터베이스 조회가 필요한 권한 검증은 Guards가 아닌 API 메서드
  내부에서 처리하는 것이 좋습니다. Guards는 간단한 인증/권한 확인만 담당해야 합니다.
</Warning>
...
export const rolePermissions: Record<string, Permission[]> = {
  ...
  moderator: [Permission.POST_READ, Permission.POST_WRITE, Permission.POST_DELETE],
...
class UserModel extends BaseModelClass {
  ...
  async updateUser(params: {
    ...
    // 본인이 아니고 관리자도 아니면 거부
    if (context.user!.id !== userId && !hasPermission(context.user, Permission.USER_WRITE)) {
...
class PostModel extends BaseModelClass {
  ...
  async update(params: {
    ...
    // 수정
    const wdb = this.getPuri("w");
    await wdb.table("posts").where("id", postId).update({
      title,
      content,
      updated_at: new Date(),
    });

    const updatedPost = await rdb.table("posts").where("id", postId).first();
  ...
  async remove(postId: number): Promise<{ success: boolean }> {
    ...
    // 소유권 확인
    if (post.user_id !== context.user!.id && context.user!.role !== "admin") {
      throw new Error("Permission denied");
    }
...
<Warning>
  **Guards 사용 시 주의사항**: 1. Guards는 인증(Authentication)만 담당, 인가(Authorization)는
  비즈니스 로직에서 처리 2. 민감한 데이터는 Guards 통과 후에도 추가 검증 필수 3. Guards 실패 시 403
  Forbidden 반환 (401 Unauthorized 아님) 4. 여러 Guards 사용 시 순서 고려 (일반적인 것부터 구체적인
  순서로) 5. Guards에서는 복잡한 비즈니스 로직 지양 (단순 권한 확인만) 6. 리소스 소유권 확인은 API
  메서드 내부에서 처리
</Warning>
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    href="/ko/api-development/authentication/session-management"
  >
    세션과 토큰 관리
  </Card>

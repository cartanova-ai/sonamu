...
<Info>Sonamu는 Zod v4를 지원합니다. 최신 버전을 설치하세요.</Info>
...
<Warning>
  **Zod 검증 사용 시 주의사항**: 1. parse()는 검증 실패 시 예외 발생 2. safeParse()는 결과 객체 반환
  3. Transform은 검증 후 실행됨 4. refine()은 마지막에 실행됨 5. 성능을 위해 스키마를 재사용
</Warning>

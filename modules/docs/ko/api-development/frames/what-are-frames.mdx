...
> {
  constructor() {
    super("User", userSubsetQueries, userLoaderQueries); // ← Entity와 연결
  }
...
<Info>Frame은 Entity 정의 없이 `.frame.ts` 파일만 있으면 됩니다.</Info>
...
<CardGroup cols={2}>
  <Card title="타입 안전성 감소" icon="triangle-exclamation">
    Entity 타입 미사용 수동 타입 관리
  </Card>
  <Card title="자동 생성 미지원" icon="circle-xmark">
    Subset, Types 자동 생성 없음 수동 작성 필요
  </Card>
</CardGroup>

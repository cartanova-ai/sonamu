...
<Info>Sonamu의 `@upload` 데코레이터는 Fastify multipart를 자동으로 처리합니다.</Info>
...
<Warning>
  **파일 업로드 설정 시 주의사항**: 1. Fastify multipart 플러그인 필수 등록 2. `@upload`
  데코레이터는 `@api` 없이 독립적으로 사용 3. 파일 크기 제한 설정 4. MIME 타입 검증 5. Storage 설정
  확인
</Warning>

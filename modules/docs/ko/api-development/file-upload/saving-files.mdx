...
class ImageModel extends BaseModelClass {
  @upload()
  async uploadOptimized(params: { image: UploadedFile }): Promise<{
    originalUrl: string;
    thumbnailUrl: string;
    mediumUrl: string;
  }> {
...
class FileModel extends BaseModelClass {
  @upload()
  async uploadWithBackup(params: { file: UploadedFile }): Promise<{
    primaryUrl: string;
    backupUrl: string;
  }> {
    const { file } = params;
...
class FileModel extends BaseModelClass {
  @upload()
  async uploadLarge(params: { file: UploadedFile }): Promise<{ url: string }> {
    const { file } = params;

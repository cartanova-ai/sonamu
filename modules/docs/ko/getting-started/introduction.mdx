...
<Frame caption="🎬 필요: Puri로 타입 안전한 쿼리 작성 - 테이블명, 칼럼명 자동완성"></Frame>
...
<Card title="Puri 가이드" icon="database" href="/ko/database/puri">
  타입 안전한 쿼리 작성하기
</Card>
...
Vite + React 기반의 서버 사이드 렌더링을 지원합니다. `registerSSR`로 SSR할 라우트를 등록하고, `preload` 콜백으로 서버에서 필요한 데이터를 미리 로드합니다.
...
registerSSR({
  path: "/users/:id",
  preload: (params) => [
    // Service 메서드 호출 → SSRQuery 반환
    UserService.getUser("C", parseInt(params.id)),
  ],
});
```

등록된 라우트는 서버에서 렌더링되고, 클라이언트에서는 자동으로 hydration됩니다. SEO 최적화와 초기 로딩 속도를 개선할 수 있습니다.
...
<CardGroup cols={3}>
  ...
  <Card title="SSR" icon="server" href="/ko/frontend-integration/ssr">
    서버 사이드 렌더링
  </Card>
</CardGroup>
...
<CardGroup cols={2}>
  <Card title="Fixture 시스템" icon="box" href="/ko/testing/writing-tests/test-helpers">
    테스트 데이터 관리
  </Card>
  <Card title="Naite 가이드" icon="chart-line" href="/ko/testing/naite">
    데이터 추적 & 시각화
  </Card>
</CardGroup>
...
<CardGroup cols={2}>
  <Card
    ...
    href="/ko/advanced-features/vector-search/pgvector-setup"
  >
    임베딩 검색 구현하기
  </Card>
  <Card title="SSE 스트리밍" icon="signal-stream" href="/ko/advanced-features/sse">
    실시간 이벤트 전송
  </Card>
</CardGroup>
...
- ✅ HMR로 서버 재시작 없이 개발
- ✅ Sonamu UI로 시각적 Entity 관리
- ✅ Syncer가 모든 코드 자동 동기화
- ✅ 변경 즉시 타입 에러 감지

<CardGroup cols={3}>
  <Card title="자세히 보기" icon="arrow-right" href="/ko/getting-started/seamless-dx">
    Seamless DX 상세 페이지
  </Card>
  <Card title="HMR 동작 원리" icon="bolt" href="/ko/tools-and-cli/hmr/how-hmr-works">
    HMR 시스템 이해하기
  </Card>
  <Card title="Sonamu UI" icon="window" href="/ko/tools-and-cli/sonamu-ui/getting-started">
    UI 사용법 가이드
  </Card>
</CardGroup>
...
export const processOrder = workflow(
  ...
  async ({ input, step }) => {
    // Step 1: 검증
    await step.define({ name: "validate" }, async () => validateOrder(input)).run();

    // Step 2: 결제
    const payment = await step.define({ name: "charge" }, async () => chargePayment(input)).run();

    // Step 3: 주문 생성
    const order = await step
      .define({ name: "create-order" }, async () => createOrderRecord(input, payment))
      .run();

    // Step 4: 이메일 전송
    await step.define({ name: "send-email" }, async () => sendConfirmationEmail(order)).run();
...
N+1 문제 자동 해결과 쿼리 최적화가 기본으로 제공됩니다.
...
<CardGroup cols={2}>
  <Card title="BentoCache" icon="layer-group" href="/ko/advanced-features/caching/bentocache">
    다층 캐싱 전략
  </Card>
  <Card title="i18n" icon="language" href="/ko/advanced-features/i18n/overview">
    다국어 지원 시스템
  </Card>
</CardGroup>
...
# Sonamu UI 열기
open http://localhost:34900/sonamu-ui
```

<CardGroup cols={2}>
  <Card title="Quick Start" icon="rocket" href="/ko/getting-started/quick-start">
    5분 만에 첫 API 만들기
  </Card>

{" "}

<Card title="Installation" icon="download" href="/ko/getting-started/installation">
  자세한 설치 가이드
</Card>

{" "}

<Card title="Core Concepts" icon="book" href="/ko/core-concepts/how-sonamu-works">
  Sonamu 작동 원리 이해하기
</Card>

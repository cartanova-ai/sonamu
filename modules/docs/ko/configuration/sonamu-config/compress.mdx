...
- `false`: `@api({ compress })` 옵션으로 API별 제어
...
`global: false`로 설정했다면 `@api()` 데코레이터의 `compress` 옵션으로 API별로 압축을 제어할 수 있습니다.

```typescript
import { api } from "sonamu";

export class DataModel {
  // 압축 적용 (전역 compress 옵션을 그대로 사용)
  @api({ compress: true })
...
→ [API별 압축 제어 사용법](/ko/advanced-features/compress/per-api-control)
...
export default defineConfig({
  server: {
    plugins: {
      compress: {
        global: false, // @api()의 compress 옵션으로 제어
        threshold: 1024, // 1KB 이상만
        encodings: ["gzip"],
      },
    },
...
export class ArticleModel {
  @api({ compress: true }) // 큰 목록 응답
  static async list() {
    return this.getPuri("r").table("articles").select("*");
  }

  @api({ compress: true }) // 큰 상세 데이터
...
// ✅ 좋은 예: API별로 제어
compress: {
  global: false,  // 필요한 것만 @api({ compress: true })
}
```

...
| `organization` | 조직/팀 관리 | Organization, Member, Invitation, Team, TeamMember 테이블, Session.active_organization_id, Session.active_team_id |
| `anonymous` | 익명 사용자 | User.is_anonymous |
...
<Note>
Passkey 플러그인 사용 시 `@better-auth/passkey` 패키지가 필요합니다.
</Note>
...
<Note>
SSO 플러그인 사용 시 `@better-auth/sso` 패키지가 필요합니다.
</Note>

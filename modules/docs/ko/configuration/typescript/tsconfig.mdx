...
- my-project/
  - api/
    - tsconfig.json          # API 서버용 설정
    - tsconfig.schemas.json  # 스키마 생성용
    - tsconfig.types.json    # 타입 생성용
  - web/
    - tsconfig.json          # React 앱용 설정
    - tsconfig.node.json     # Vite 설정용
</FileTree>

<Info>
API와 Web은 각각 독립적인 TypeScript 프로젝트입니다. 설정이 서로 분리되어 있습니다.
</Info>
...
<Tabs>
  ...
  <Tab title="데코레이터" icon="sparkles">
    ...
    **사용 예시:**
    ...
    <Info>
    Entity는 `entity.json` 파일로 정의하며, TypeScript 데코레이터는 사용하지 않습니다. 자세한 내용은 [Entity 정의하기](/core-concepts/entity/defining-entities)를 참고하세요.
    </Info>
  </Tab>
  ...
  <Tab title="Strict 모드" icon="shield-check">
    ...
    자세한 내용은 [타입 체크](/configuration/typescript/type-checking)를 참고하세요.
  </Tab>
...
<Info>
API는 `moduleResolution: "bundler"` (소문자), Web은 `moduleResolution: "Bundler"` (대문자)를 사용합니다. 둘 다 Vite가 번들러 방식으로 모듈을 해석합니다.
</Info>
...
자세한 내용은 [Path Mapping](/configuration/typescript/path-mappings)을 참고하세요.
...
<Tip>
개발 중에는 `--watch` 모드로 실시간 타입 체크를 권장합니다.
</Tip>
...
<Warning>
이 옵션들은 기존 코드와 호환성 문제를 일으킬 수 있습니다.
</Warning>

...
<Tip>`@/*` 하나만 사용하는 것을 권장합니다. 너무 많은 매핑은 오히려 혼란을 줄 수 있습니다.</Tip>
...
<Info>API에서 path mapping이 필요하다면 추가할 수 있지만, 일반적으로 불필요합니다.</Info>
...
<Info>Sonamu는 `package.json`의 `exports` 필드로 서브패키지를 제공합니다.</Info>
...
<Tip>`type` import는 컴파일 후 제거되어 번들 크기를 줄입니다.</Tip>
...
<Card
  title="컴포넌트 작성"
  icon="cube"
  href="/ko/frontend-integration/react-components/generated-components"
>
  React 컴포넌트를 작성하세요
</Card>

...
<Step title="환경변수 설정" icon="pen">
  `.env` 파일을 열어 실제 값으로 수정합니다. ```bash # 예시 DB_HOST=localhost DB_PORT=5432
  DB_USER=postgres DB_PASSWORD=your-db-password ```
</Step>
...
<Info>주석은 `#`으로 시작합니다. 빈 줄은 무시됩니다.</Info>
...
<Tab title="스테이징" icon="flask">
  ```bash title=".env.staging" # 테스트 서버용 설정 PROJECT_NAME=MyProject-Staging
  DB_HOST=staging-db.example.com DB_PORT=5432 DB_USER=myproject_user
  DB_PASSWORD=your-staging-password # 최소 32자 DATABASE_NAME=myproject_staging
  SESSION_SECRET=your-staging-session-secret # 최소 64자 SESSION_SALT=your-staging-session-salt
  DRIVE_DISK=s3 AWS_ACCESS_KEY_ID=your-staging-aws-key-id
  ...
  **특징:** - 원격 데이터베이스 - S3 스토리지 - 프로덕션과 유사한 환경
</Tab>
...
<Tip>
  환경변수가 설정되지 않았을 때만 기본값을 사용하려면 `??`를 사용하세요. `||`는 빈 문자열도 falsy로
  처리합니다.
</Tip>
...
<CardGroup cols={2}>
  <Card
    title="데이터베이스 인증 정보"
    icon="database"
    href="/ko/configuration/environment-variables/database-credentials"
  >

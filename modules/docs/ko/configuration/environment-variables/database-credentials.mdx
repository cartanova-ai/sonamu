...
<Tabs>
  ...
  <Tab title="프로덕션" icon="server">
    ...
    **읽기 복제본 설정 (선택):**
    ...
    <Info>
    Sonamu는 `environments.production_slave` 설정을 자동으로 인식하여 읽기 전용 쿼리에 사용합니다. 자세한 내용은 [데이터베이스 설정](/configuration/sonamu-config/database)을 참고하세요.
    </Info>
  </Tab>
</Tabs>
...
<CardGroup cols={2}>
  ...
  <Card 
    title="데이터베이스 설정" 
    icon="gear" 
    href="/configuration/sonamu-config/database"
  >
    데이터베이스 상세 설정을 확인하세요
  </Card>
</CardGroup>

...
<Info>Sonamu v2부터 PostgreSQL을 공식 지원하며, MySQL에서 마이그레이션을 지원합니다.</Info>
...
<Warning>`DB_PASSWORD`는 절대 코드에 하드코딩하지 마세요! 환경변수로만 관리해야 합니다.</Warning>
...
<Info>
  `password`는 기본값을 설정하지 마세요. 환경변수가 없으면 `undefined`로 두어 명시적으로 에러가
  발생하도록 합니다.
</Info>
...
<Tip>트래픽이 많은 프로덕션 환경에서는 `max`를 20-50으로 증가시키세요.</Tip>
...
<Card title="마이그레이션" icon="database" href="/ko/database/migrations">
  데이터베이스 스키마를 관리하세요
</Card>

  <Card title="데이터베이스 설정" icon="gear" href="/ko/configuration/sonamu-config/database">
    데이터베이스 상세 설정을 확인하세요
  </Card>
</CardGroup>

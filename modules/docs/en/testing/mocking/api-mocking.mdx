...
<Info>
  **bootstrap() and timers**: Sonamu's `bootstrap()` function automatically calls
  `vi.useRealTimers()` in `afterEach` to restore timers. No need to call it separately.
</Info>
...
test("check multiple files", async () => {
  const files = ["/uploads/file1.pdf", "/uploads/file2.pdf", "/uploads/file3.pdf"];
...
async function access(path: string) {
  ...
  const mocked = Naite.get("mock:fs/promises:virtualFileSystem").result().includes(path);
...
test("payment failure scenario", async () => {
  ...
  await expect(orderService.processPayment(order.id)).rejects.toThrow("Card declined");
});
```
...
test("fetch weather info", async () => {
  ...
  expect(axios.get).toHaveBeenCalledWith(expect.stringContaining("Seoul"));
});
...
test("API error handling", async () => {
  ...
  await expect(weatherService.getCurrentWeather("Seoul")).rejects.toThrow(
...
test("image upload and processing", async () => {
  ...
  const exists = await imageService.checkBothExist(originalPath, thumbnailPath);
  expect(exists).toBe(true);

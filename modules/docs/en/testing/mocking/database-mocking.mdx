...
test("duplicate username error", async () => {
  ...
  await expect(userModel.create({ username: "john" })).rejects.toThrow("Username already exists");
...
test("constraint violation", async () => {
  ...
  await expect(userModel.create({ username: null as any })).rejects.toThrow();
...
3. **DDL statements (exception for PostgreSQL)**
   ...
   test("create table", async () => {
     ...
     // PostgreSQL: ✅ DDL inside transactions is also rolled back
   });
   ```
...
See [API Mocking](/en/testing/mocking/api-mocking) for details.
...
4. **Limit parallel execution**: Sequential test execution is recommended for transaction isolation.
   </Warning>

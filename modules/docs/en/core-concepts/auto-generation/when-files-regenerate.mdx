...
When you save a `.types.ts` file, it's copied to targets.

### Regenerated Files

<Steps>
<Step title="Target Copy">
Modified Types file is copied to target projects
...
<AccordionGroup>
  <Accordion title="Change Only Necessary Files">
    If you only modified Types, no need to re-save Entity
    
    - Types change → Target copy only (fast)
    - Entity change → All schemas regenerated (slow)
  </Accordion>

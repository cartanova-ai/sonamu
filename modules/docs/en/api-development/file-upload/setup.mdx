...
<Info>Sonamu's `@upload` decorator automatically handles Fastify multipart.</Info>
...
<Warning>
  **Cautions when setting up file upload**: 1. Fastify multipart plugin must be registered 2.
  `@upload` is used independently (without `@api`) 3. Set file size limits 4. MIME type validation
...
<CardGroup cols={2}>
  <Card
    title="UploadedFile Class"
    icon="file"
    href="/en/api-development/file-upload/uploaded-file-class"
  >

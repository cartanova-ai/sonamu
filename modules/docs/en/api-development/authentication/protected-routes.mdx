...
<Info>
  The specific implementation of `query` Guard may vary by project. It can be extended with various
  methods like API keys, tokens, signatures, etc.
</Info>
...
<Warning>
  **Note**: For permission verification requiring complex business logic or database queries, it's
  better to handle it inside API methods rather than in Guards. Guards should only handle simple
...
<Warning>
  **Cautions when using Guards**: 1. Guards only handle Authentication, Authorization should be
  handled in business logic 2. Additional verification required for sensitive data even after
  passing Guards 3. Return 403 Forbidden on Guard failure (not 401 Unauthorized) 4. Consider order
  when using multiple Guards (general to specific) 5. Avoid complex business logic in Guards (simple
  permission checks only) 6. Resource ownership verification should be handled inside API methods
</Warning>
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    href="/en/api-development/authentication/session-management"
  >

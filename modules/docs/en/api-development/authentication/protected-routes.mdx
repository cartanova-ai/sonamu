...
<CardGroup cols={2}>
  ...
  <Card title="Built-in Guards" icon="shield-halved">
    admin, user, query
...
Sonamu provides three Guards by default.
...
You can create custom Guards that match your project's business logic.
...
export const guards: Record<string, GuardFunction> = {
  ...
  /**
   * admin Guard: Admin permission required
   */
  admin: (context) => {
  ...
  /**
   * query Guard: Query parameter verification
   */
  query: (context) => {
    // Implementation varies by project
  ...
  /**
   * verified Guard: Email verified users only
   */
  verified: (context) => {
  ...
  /**
   * premium Guard: Premium members only
   */
  premium: (context) => {
    if (!context.user) return false;
  ...
  /**
   * owner Guard: Resource ownership check (dynamic)
   */
  owner: async (context) => {
    // This Guard needs resource ID as parameter
    // So verifying inside API method is more appropriate
    return true;
  },
};
```
...
Create middleware to integrate Guards with Fastify.
 ...
 * Guards middleware
 * 
 * Checks API's guards option and executes each Guard function.
 */
export async function guardsMiddleware(
  request: FastifyRequest,
  reply: FastifyReply
) {
  // Find API by current request path and method
...
### Registering Middleware on Server
...
/**
 * Permission-based Guard creation function
 */

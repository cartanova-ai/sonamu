...
<Warning>
  **Session/Token Management Cautions**: 1. Manage Session Secret and JWT Secret via environment
  ...
  in database to enable invalidation 4. Set Access Token short, Refresh Token long 5. Set `secure:
  true` only when using HTTPS 6. `httpOnly: true` is required for XSS prevention 7. Use Redis TTL
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="shield-halved"
    href="/en/api-development/authentication/protected-routes"
  >

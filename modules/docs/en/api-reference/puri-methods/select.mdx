...
literals as untyped parameters and does not treat column-like strings as columns. A standalone
literal-only `Puri.concat()` expression is therefore not runnable on PostgreSQL. Use
`Puri.concatExpressions()` to combine string SQL expressions with literal text.
...
For a plain column or non-null string expression, similarity functions return
`SqlExpression<"number">`. These expressions can be combined with `rawNumber` to compute
per-column weighted scores.
...
<Info>
The result preserves the input expression's nullability. `jsonText()` produces a nullable string
expression; passing it to a similarity helper produces `SqlExpression<"number", true>` and selects as `number | null`; a plain
column or non-null expression produces `SqlExpression<"number">` and selects as `number`. The
  ...
  <Tab title="Users + Posts" icon="user-pen">
    ...
    const users = await puri.table("users")
      ...
      .select({
        ...
        // Calculated field from typed string expressions

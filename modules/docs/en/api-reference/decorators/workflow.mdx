...
<Warning>
  `workflow` is a function, not a TypeScript decorator. Applying `@workflow(...)` above a function
  declaration is not supported: decorators can only be applied to classes and class members.
</Warning>

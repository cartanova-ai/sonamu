...
<Info>Without cache configuration, using `@cache` will cause an error.</Info>
...
<Tip>String format is more readable: `"10s"`, `"5m"`, `"1h"`, `"1d"`</Tip>
...
<Info>
  Advanced options like `grace` and `timeouts` are provided by BentoCache's `RawCommonOptions`. See
  [BentoCache documentation](https://bentocache.dev) for details.
</Info>
...
<Warning>Decorator order: `@api` → `@cache` → `@transactional`</Warning>

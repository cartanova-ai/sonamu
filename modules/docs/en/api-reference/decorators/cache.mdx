...
Grace period that returns stale values for a certain time even after the cache expires (Stale-While-Revalidate pattern).
...
async getData() {
  // 0-10min: Return fresh cache
  // 10-15min: Return stale cache immediately + revalidate in background
  // After 15min: Cache miss, compute fresh value
}
```

**How it works**:
- Within TTL: Returns fresh cached value
- After TTL expires (within grace period): Returns stale cache immediately, refreshes in background
- After grace period expires: Cache miss

<Info>
Stale-While-Revalidate is a pattern that provides fast responses to users (even if stale) while updating the cache in the background so the next user receives fresh data.
</Info>

...
<Info>
`sonamuFilter` is automatically generated based on the Entity's props. Relations and virtual fields are excluded from filtering targets.
</Info>
...
Only fields defined in the Entity's props can be filtered. Relation fields and virtual fields are excluded from filtering targets.

```typescript
// ❌ Error: Field 'employee' cannot be filtered
await ProjectModel.findMany("A", {
  sonamuFilter: { employee: { eq: 1 } }  // relation field
});

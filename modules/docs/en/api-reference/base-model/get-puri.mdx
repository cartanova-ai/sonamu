...
`getPuri` automatically detects and reuses transaction contexts. This is a critical feature when used with the `@transactional` decorator.

### How It Works

The `getPuri` method internally operates as follows:

1. Checks for active transaction via `DB.getTransactionContext()`
2. If transaction exists, reuses that transaction
3. If no transaction, creates a new PuriWrapper

```typescript
// Internal getPuri logic (for reference)
async getPuri(which: DBPreset): PuriWrapper {
  // 1. Attempt to get transaction from context
  const trx = DB.getTransactionContext().getTransaction(which);

  if (trx) {
    // 2. Reuse if transaction exists
    return trx;
  }

  // 3. Return new PuriWrapper if no transaction
...
class UserModelClass extends BaseModelClass {
  ...
  async createUserWithProfile(params: {
    ...
    // Call getPuri inside @transactional
    const wdb = this.getPuri("w");

    // wdb automatically uses transaction created by @transactional
...
This feature ensures safety when calling other methods within a transaction. All `getPuri` calls use the same transaction.
...
class UserModelClass extends BaseModelClass {
  ...
  async createUser(email: string, name: string) {
    ...
    // Call another method - uses same transaction
    await this.createDefaultProfile(user.id);

    return user;
  }

  // No @transactional, but uses same transaction when called from above
  async createDefaultProfile(userId: number) {
    const wdb = this.getPuri("w");

    // Automatically reuses createUser's transaction
    await wdb.table("profiles")
      .insert({
        user_id: userId,
        bio: "Welcome!"
...
<Info>
The transaction context set by `@transactional` decorator is automatically shared across all nested method calls. This is implemented via AsyncLocalStorage.
</Info>

<Warning>
PuriWrapper obtained via `getPuri` within a transaction and a new transaction started with `wdb.transaction()` are **different transactions**. Nested transactions must be managed explicitly if needed.
</Warning>

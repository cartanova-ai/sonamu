...
export class ValidationException extends SoException {
  ...
  static fieldInvalid(fieldName: string, reason: string) {
    return new ValidationException(`${fieldName} is invalid: ${reason}`, {
      field: fieldName,
      code: "INVALID",
      reason,
    });
  ...
  static fieldTooShort(fieldName: string, minLength: number) {
    return new ValidationException(`${fieldName} must be at least ${minLength} characters`, {
      field: fieldName,
      code: "TOO_SHORT",
      minLength,
    });
...
// ✅ Good
throw new CustomException("Cannot process order: insufficient stock", {
  productId: 123,
  available: 5,
  requested: 10,
});
...
- [SoException](/en/api-reference/exceptions/sonamu-error)
- [BadRequestException](/en/api-reference/exceptions/bad-request-error)
- [UnauthorizedException](/en/api-reference/exceptions/unauthorized-error)

...
<Info>`migrate run` executes all pending migrations in order.</Info>
...
| --- | ---------------------------------- | ----- | ------------------- |
| 1   | 20251220143022_create\_\_users.ts  | 1     | 2025-12-20 14:30:22 |
| 2   | 20251220143100_create\_\_posts.ts  | 1     | 2025-12-20 14:30:22 |
...
<Warning>
  **Before running in production**: 1. Test in staging first 2. Confirm backup is complete 3. Plan
  for downtime 4. Prepare rollback plan
</Warning>
...
<Info>Some DDL operations may not support transactions.</Info>
...
<Warning>
  When a migration fails: - Only that migration is rolled back - Previous migrations are retained -
...
export async function up(knex: Knex): Promise<void> {
  await knex.raw(`ALTER TABLE users ALTER COLUMN new_field SET NOT NULL`);
}
```

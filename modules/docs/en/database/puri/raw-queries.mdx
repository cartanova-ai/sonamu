...
const results = await db.table("documents").select({
  highlighted: Puri.rawString(
    "pgroonga_highlight_html(??, ARRAY[?])",
    ["documents.title", "search term"],
  ),
});
...
<Warning>
  **Don't confuse `??` and `?`.** Using `?` for a column name will escape it as a value, causing SQL errors.
  Conversely, using `??` for user input will escape it as an identifier, leading to unintended behavior.
</Warning>

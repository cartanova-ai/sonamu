...
| Placeholder | Purpose            | Escaping                                   |
| ----------- | ------------------ | ------------------------------------------ |
| `?`         | Value binding      | Escaped as value (`'hello'`)               |
| `??`        | Identifier binding | Escaped as identifier (`"table"."column"`) |
...
// SQL: pgroonga_highlight_html("documents"."title", ARRAY['search term'])
```

<Warning>
  **Don't confuse `??` and `?`.** Using `?` for a column name will escape it as a value, causing SQL
  errors. Conversely, using `??` for user input will escape it as an identifier, leading to
  unintended behavior.
</Warning>
...
const results = await db.table("messages").select({
  greeting: Puri.concatExpressions("Hello", ", ", "Sonamu"),
});
```
...
`Puri.concatExpressions()` adds PostgreSQL `text` casts to bound literal parameters, so
literal-only expressions like the example above are runnable. Legacy `Puri.concat()` binds
literals as untyped parameters and does not interpret an argument such as `"users.name"` as a
column. A standalone literal-only `Puri.concat()` expression is therefore not runnable on
PostgreSQL. To combine string SQL expressions with literal text, use `Puri.concatExpressions()`
and `Puri.coalesce()` as shown in [JSON Field Extraction](#json-field-extraction).
...
<Warning>
  **SQL Injection Warning**: Always use bindings (`?`) with `whereRaw`. Never put user input
  directly in strings.
</Warning>
...
```typescript
const query = db.table("documents");
const title = query.jsonText("metadata", "title");
const description = query.jsonText("metadata", "document", "description");

const results = await query.select({
  title,
  summary: Puri.concatExpressions(
    Puri.coalesce(title, "Untitled"),
    " — ",
    Puri.coalesce(description, "No description"),
  ),
});
```

`jsonText()` accepts a generated JSON column and one or more path segments. It returns a string SQL
expression without changing the query. PostgreSQL produces SQL `NULL` when the path is missing or
its JSON value is `null`, so selecting `jsonText()` directly infers `string | null`. `Puri.coalesce()`
turns that nullable expression into a non-null `string`, as `summary` uses above. Path segments,
literal text, and fallback values are bound as values.
`Puri.concatExpressions()` accepts only literal strings and string SQL expressions.
...
const results = await db
  .table("documents")
  .whereJsonKeyExists("metadata", "nullable_key")
  .whereGroup((group) => {
    group
      .whereJsonKeyExists("metadata", "title")
      .orWhereJsonKeyExists("metadata", "description");
  });
```

`whereJsonKeyExists()` uses PostgreSQL's JSONB `?` operator and binds the key as a value. It checks
whether the top-level key exists, so a key whose JSON value is `null` is still present. Use
`whereJsonKeyExists()` and `orWhereJsonKeyExists()` inside a group to keep AND/OR precedence
explicit.
...
<Warning>
  **Cautions when using Hydrate**: 1. **Field naming**: JOIN fields must use `__` (double
  underscore) 2. **Manual call**: Raw Puri queries require manual hydrate() call 3. **Type safety**:
  Types after hydrate must be manually defined 4. **Performance**: hydrate has runtime overhead, so
  use Subset queries for simple cases
</Warning>

<Info>
  **Recommendations**: - Use **Subset queries** when possible (automatic hydrate) - Only use manual
  hydrate when complex Raw SQL is needed - Apply field naming conventions (`__`) consistently
</Info>

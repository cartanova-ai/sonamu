...
const results = await db.table("messages").select({
  greeting: Puri.concat("Hello", ", ", "Sonamu"),
});
```
...
`Puri.concat()` binds every argument as a string value. It does not interpret an argument such as
`"users.name"` as a column. To combine string SQL expressions with literal text, use
`Puri.concatExpressions()` and `Puri.coalesce()` as shown in [JSON Field
Extraction](#json-field-extraction).

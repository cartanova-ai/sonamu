...
<Info>
When the `transaction()` callback function completes successfully, it automatically COMMITs.
If an error occurs, it automatically ROLLBACKs.
</Info>
...
- Independent rollback of partial operations
- Structure complex business logic
- Error isolation possible
</Info>
...
<Warning>
Code continues to execute even after calling `rollback()`.
You must explicitly `return` or `throw`.
</Warning>
...
<Warning>
**Transaction Optimization Principles**:
- Keep transactions as short as possible
- External API calls outside transactions
- Read-only queries don't need transactions
- Minimize lock wait time
</Warning>
...
<CardGroup cols={2}>
  ...
  <Card title="Best Practices" icon="star" href="/database/transactions/best-practices">
  ...
  <Card title="Puri" icon="search" href="/database/puri/basic-queries">
    Using query builder
  </Card>
</CardGroup>

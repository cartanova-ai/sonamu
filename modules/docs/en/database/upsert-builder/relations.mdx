...
<Info>
**ManyToMany Save Order**:
1. Save both main tables first (projects, employees)
2. Save junction table later (projects__employees)
</Info>
...
<Warning>
Self-referencing refers to the same table, so it can be saved with **one ubUpsert**.
However, circular references are not possible.
</Warning>
...
<CardGroup cols={2}>
  ...
  <Card title="Basic Usage" icon="play" href="/database/upsert-builder/basic-usage">

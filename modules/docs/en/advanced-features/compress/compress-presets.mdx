...
<Tab title="When to Use" icon="lightbulb">
  - General API servers - Most cases (recommended) - When no special optimization is needed
</Tab>
...
<Tab title="When to Use" icon="lightbulb">
  - Environments with expensive bandwidth - Mobile network targets - Minimizing CDN transfer volume
...
<Tab title="When to Use" icon="lightbulb">
  - Environments with limited CPU resources - High TPS (Transactions Per Second) - Minimizing
  response latency - Compressing only large responses
</Tab>
...
<Tab title="When to Use" icon="lightbulb">
  - Legacy browser support (IE) - Stability priority - Browser compatibility issues - Real-time
  compression (fast processing)
</Tab>
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="gauge-high"
    href="/en/advanced-features/compress/performance-optimization"
  >

...
Use `jsonText()` for text extraction, `.whereJsonSupersetOf()` for containment, and
`.whereJsonKeyExists()` for top-level key existence. Use `Puri.rawString()` and parameterized
`.whereRaw()` only for JSONB operations that these helpers do not support.

**JSONB text extraction (->>):**

```typescript
const query = ProductModel.getPuri("r").table("products");
const brand = query.jsonText("products.metadata", "brand");

const products = await query.select({
  id: "products.id",
  name: "products.name",
  brand, // string | null
  brandWithFallback: Puri.coalesce(brand, "Unknown"), // string
});
```

`jsonText()` also accepts multiple path segments for nested text. A missing path or JSON `null`
produces SQL `NULL`, so a direct selection is `string | null`; `Puri.coalesce()` provides a
non-null fallback.
...
const productsWithDiscount = await ProductModel.getPuri("r")
  .table("products")
  .select("*")
  .whereJsonKeyExists("products.metadata", "discount");
```

`whereJsonKeyExists()` checks only top-level keys. A key still exists when its JSON value is
`null`.
...
// metadata->'specs'->>'cpu'
const query = ProductModel.getPuri("r").table("products");
const laptops = await query.select({
  id: "products.id",
  name: "products.name",
  cpu: query.jsonText("products.metadata", "specs", "cpu"),
  ram: query.jsonText("products.metadata", "specs", "ram"),
});
```
...
- For unsupported operations, use raw expressions with parameter binding to prevent SQL injection

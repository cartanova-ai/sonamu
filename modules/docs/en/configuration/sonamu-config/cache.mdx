...
<Tip>If multiple apps share the same Redis instance, set a prefix to prevent key collisions.</Tip>
...
<Note>`redisBus` can use the same connection as the `redis` driver.</Note>
...
→ [Cache Decorator Usage](/en/advanced-features/caching/cache-decorator)
...
- [cache decorator](/en/advanced-features/caching/cache-decorator) - Apply caching to APIs
- [cache-control](/en/configuration/sonamu-config/cache-control) - HTTP cache control
- [cache strategies](/en/advanced-features/caching/cache-strategies) - Advanced caching strategies

...
<Tip>
  When using plugins, you also need to enable them in `sonamu.config.ts`. See the [Plugin
  Configuration](#plugin-configuration) section below for details.
</Tip>
...
<Tip>
  For the complete API list, refer to the [better-auth
  documentation](https://www.better-auth.com/docs/api-reference).
</Tip>
...
<Note>The `@better-auth/passkey` package is required to use the Passkey plugin.</Note>
...
<Note>The `@better-auth/sso` package is required to use the SSO plugin.</Note>
...
<Note>
  Each plugin's schema (`*_SCHEMA`) maps Sonamu's snake_case column names to better-auth's camelCase
  field names. It must be passed along with the corresponding plugin.
</Note>

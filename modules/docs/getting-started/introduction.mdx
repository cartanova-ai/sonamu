...
<Frame caption="📸 필요: VS Code에서 타입 에러가 빨간 줄로 표시되는 스크린샷 (user.nonExistent에 빨간 밑줄)"></Frame>
...
<Frame caption="🎬 필요: 백엔드 @api 메서드 작성 → pnpm generate → 프론트엔드에서 useQuery 사용 (20초)"></Frame>
...
<Frame caption="📸 필요: Naite Viewer 화면 - 테스트 실행 중 데이터베이스 변화를 시각적으로 보여주는 UI"></Frame>
...
<Frame caption="📸 필요: Entity에 vector 필드 정의 → 자동 생성된 벡터 검색 메서드 사용하는 화면"></Frame>
...
<Frame caption="🎬 필요: Sonamu UI에서 AI에게 '이커머스 주문 시스템 만들어줘' 입력 → Entity 4개 자동 생성 (30초)"></Frame>
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    href="/advanced-features/workflows"
  >
    비즈니스 로직 관리
  </Card>
</CardGroup>
...
<Frame caption="📸 필요: Sonamu UI에서 User Entity를 정의하는 화면"></Frame>
...
<Frame caption="🎬 필요: 위 전체 플로우를 실제로 시연하는 영상 (30초 타임랩스)"></Frame>
...
<Frame caption="📸 필요: 터미널에서 pnpm create sonamu 실행 후 프로젝트 생성되는 전체 과정"></Frame>
...
<CardGroup cols={2}>
  ...
  <Card
    title="Core Concepts"
    icon="book"
    href="/core-concepts/how-sonamu-works"
  >
    Sonamu 작동 원리 이해하기
  </Card>

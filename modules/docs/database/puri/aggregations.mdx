...
`Puri.count("column")`은 해당 컬럼이 NULL이 아닌 행만 셉니다.
</Info>
...
<Info>
**WHERE vs HAVING**:
- `WHERE`: GROUP BY **전**에 필터링 (개별 행 대상)
- `HAVING`: GROUP BY **후**에 필터링 (집계 결과 대상)
</Info>
...
<CardGroup cols={2}>
  <Card title="Raw Queries" icon="code" href="/database/puri/raw-queries">
    복잡한 SQL 직접 작성하기
  </Card>

...
const relativeJavaScriptToTypeScript = (pathname: string) =>
  pathname.replace(/\.([cm])?js(x?)$/i, ".$1ts$2");
const relativeTypeScriptToJavaScript = (pathname: string) =>
  pathname.replace(/\.([cm])?ts(x?)$/i, ".$1js$2");

/** @internal */
export const absoluteJavaScriptToTypeScript = (location: URL) =>
  new URL(
    relativeJavaScriptToTypeScript(fileNameOf(location)) + location.search + location.hash,
    location,
  );
...
export const absoluteTypeScriptToJavaScript = (location: URL) =>
  new URL(
    relativeTypeScriptToJavaScript(fileNameOf(location)) + location.search + location.hash,
    location,
  );
...
function translateLocation(location: URL, fromDir: URL, toDir: URL, convert = identity) {
  ...
  if (locationPath.startsWith(fromPath)) {
    return new URL(
      convert(locationPath.slice(fromPath.length)) + location.search + location.hash,
      toDir,
    );
  } else {
    return null;
...
export function sourceToOutput(source: URL, locations: ResolutionConfig | undefined) {
  ...
  if (locations?.outputBase) {
    ...
    return translateLocation(
      source,
      locations.sourceBase,
      locations.outputBase,
      isTs ? relativeTypeScriptToJavaScript : identity,
    );
...
export function* outputToSourceCandidates(output: URL, locations: ResolutionConfig | undefined) {
  if (locations?.outputBase) {
    if (testAnyJavaScript.test(output.pathname)) {
      const tsCandidate = translateLocation(
        output,
        locations.outputBase,
        locations.sourceBase,
        relativeJavaScriptToTypeScript,
      );
      if (tsCandidate) {
        yield tsCandidate;
      ...
      const candidate = translateLocation(
        output,
        locations.outputBase,
        locations.sourceBase,
        identity,

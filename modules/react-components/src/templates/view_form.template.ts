...
import { getEnumInfoFromColName, getRelationPropFromColName } from "./view_list.template";
...
export class Template__view_form extends Template {
  ...
  renderColumnNew(entityId: string, col: RenderingNode, _names: EntityNamesRecord): string {
    ...
    switch (col.renderType) {
      ...
      case "enums":
        try {
          ...
          return `<Select key={\`${col.name}-\${form.${col.name}}\`} ${regExpr}>
...
import { Textarea } from "@sonamu-kit/react-components/components";
...
import { ImageUploader } from "@sonamu-kit/react-components/components";
import { useTypeForm, useGoBack } from "@sonamu-kit/react-components/lib";
...
${columns
  ...
  .map((col) => {
    try {
      const relProp = getRelationPropFromColName(entityId, col.name.replace("_id", ""));
      const targetNames = EntityManager.getNamesFromId(relProp.with);
      return `import { ${relProp.with}IdAsyncSelect } from "src/components/${targetNames.fs}-test/${relProp.with}IdAsyncSelect";`;
    } catch {
      return "";
    }
  })
...
// Icons
const FormIcon = (props: any) => <IconifyIcon icon="mdi:form-select" {...props} />;
const ArrowLeftIcon = (props: any) => <IconifyIcon icon="lucide:arrow-left" {...props} />;
const SaveIcon = (props: any) => <IconifyIcon icon="lucide:save" {...props} />;
        ...
        <div className="space-y-6 mb-8">
          ...
          <div className="flex items-center justify-between">
            ...
            {mode !== "modal" && (
              <Button
                variant="outline"
                onClick={() => goBack("/admin/${names.fsPlural}")}
                className="gap-2"
              >

      ...
      case "number-fk_id": {
        try {
          const baseName = col.name.includes(".")
            ? col.name.split(".").pop()!.replace("_id", "")
...
import { ${(() => {
        ...
        const filterEnumIds = filterColumns
          ...
          .map((col) => {
            if (col.config?.enumId) return col.config.enumId;
            try {
              const { id: enumId } = getEnumInfoFromColName(entityId, col.name);
              return enumId;
            } catch {
...
// Icons
const ListIcon = (props: any) => <IconifyIcon icon="mdi:format-list-bulleted" {...props} />;
const EditIcon = (props: any) => <IconifyIcon icon="lucide:square-pen" {...props} />;
const TrashIcon = (props: any) => <IconifyIcon icon="lucide:trash-2" {...props} />;
const SearchIcon = (props: any) => <IconifyIcon icon="mdi:magnify" {...props} />;
...
export default function ${names.capital}List({}: ${names.capital}ListProps) {
  ...
  // 리스트 쿼리
  const { data, mutate, isLoading } = ${names.capital}Service.use${names.capitalPlural}("A", listParams);
  ...
  .map((col) => {
    if (col.renderType === "enums") {
      try {
        ...
        const enumId = col.config?.enumId ?? getEnumInfoFromColName(entityId, col.name).id;

...
function parseArgs(args: string[]): { files: string[]; pattern: string | undefined } {
  const files: string[] = [];
...
describe("test-command argument parsing", () => {
  it("인수 없음 시 빈 payload를 생성한다", () => {
    const { files, pattern } = parseArgs([]);
    expect(files).toEqual([]);
    expect(pattern).toBeUndefined();
  });

  it("files와 --pattern이 올바르게 파싱된다", () => {
    const { files, pattern } = parseArgs(["src/foo.test.ts", "--pattern", "my test"]);

...
export type HttpValidator = z.ZodType<Record<string, unknown>>;
...
export async function createHttpValidator(options: {
  api: ExtendedApi;
  ...
  const validator = fastifyCaster(getZodObjectFromApi(options.api, { ...options.types }));
  ...
  const compiled = jit(validator, { eager: true });

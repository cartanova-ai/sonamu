...
Traces:
  ...
  UserModel > CRUD 테스트 > Create - save로 새 유저 생성
  user.model.test.ts

    [puri:ub-register] upsert-builder.ts:178
    {
      "tableName": "users",
      "row": { "email": "newuser@test.com", ... }
    }
```
...
export default defineConfig({
  ...
  server: {
    ...
    plugins: {
      sse: true,           // (선택) SSE 활성화 시 /__test__/events 엔드포인트 추가
    },
    // ...
  },
...
| `test.devRunner.vitestConfigPath` | `undefined` | `dev-vitest-manager.ts` `start()` | Vitest 설정 파일 위치 (createVitest의 `config` 옵션) |
...
| `server.plugins.sse` | `undefined` | `dev-test-routes.ts` | SSE 엔드포인트(`/__test__/events`) 등록 여부 |
| `database.name` | — | `db.ts` `generateDBConfig()`, `global-setup.ts` | 테스트 DB 이름 (`{name}_test`) |
| `database.defaultOptions` | — | `db.ts` `generateDBConfig()` | 기본 Knex 설정 (pool, connection 등) |
| `database.environments.test` | `undefined` | `db.ts` `generateDBConfig()`, `global-setup.ts` | 테스트 환경 DB 연결 오버라이드 |

### 활성화 조건

DevRunner는 `sonamu.ts`에서 `isLocal() && config.test?.devRunner?.enabled` 조건으로 등록된다. `isLocal()`은 `LR` 환경변수가 undefined이거나 `"local"`인 경우 true를 반환한다 (`controller.ts`). 즉, 로컬 개발 환경에서만 동작하며 원격(production/staging) 환경에서는 설정과 무관하게 비활성화된다.

...
**CRITICAL: 모든 PHASE에서 중간 확인을 반드시 거친다.**
requirements에 명시되지 않은 사항은 추측하지 말고 사용자에게 확인한다. 한 번에 모든 것을 완료하려 하지 말고, 단계별로 사용자에게 결과를 보여주고 다음 단계 진행 여부를 확인한다. 특히 설계 결정(관계 유형, 필드 구성, 상태 전이 등)은 반드시 사용자 승인 후 구현에 들어간다.
...
2. **Entity 식별 및 역할 정의**
   ...
   **User/인증 관련 필수 확인 사항** (requirements에 명시되지 않았을 경우 반드시 질문):
   - "로그인 아이디는 이메일인가요, 사번/사원번호인가요, 별도 아이디가 있나요?"
   - "비밀번호 인증을 사용하나요? SSO/OAuth 등 외부 인증을 쓰나요?"
   - "사용자 역할/권한은 고정된 값(enum)인가요, 별도 Role 엔티티로 관리하나요?"
   - "한 사용자가 여러 역할을 가질 수 있나요? (BelongsToOne vs ManyToMany)"
   - "사용자 프로필에 필수인 필드는? (이름, 전화번호, 부서, 직급 등)"

   **관계 유형 필수 확인 사항** (requirements에 명시되지 않았을 경우 반드시 질문):
   - 1:N인지 N:M인지 모호한 관계는 반드시 확인
   - "이 데이터는 부모 엔티티와 함께 생성/삭제되나요, 독립적으로 관리되나요?"
   - "이력/로그 성격의 데이터가 필요한가요? (상태 변경 이력, 승인 이력 등)"

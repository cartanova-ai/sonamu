...
export interface SourceBlameDeps {
...
interface SourceBlameContributor {
  name: string;
  commits: number;
...
interface SourceBlameData {
  source: string;
  primary_owner: string;
  contributors: SourceBlameContributor[];
}

const defaultDeps: SourceBlameDeps = {
  listFileHistory,
  blameFile,
  callAi,
};
...
export async function runSourceBlame(
  ...
  project: CddProject,
  deps: SourceBlameDeps = defaultDeps,
  ...
  const [history, blame] = await Promise.all([
    deps.listFileHistory(sourceAbsPath, {
      cwd: options.cwd,
      since: options.since,
      until: options.until,
    }),
    deps.blameFile(sourceAbsPath, {
      cwd: options.cwd,
  ...
  const enriched = await enrichWithAiRoles(contributors, history, sourceAbsPath, options, deps);

  enriched.sort((a, b) => b.score - a.score);

  const primaryOwner = enriched.length > 0 ? enriched[0].name : "";

  const data: SourceBlameData = {
    source: sourcePath,
...
function scoreContributors(contributorMap: Map<string, ContributorData>): SourceBlameContributor[] {
...
async function enrichWithAiRoles(
  ...
  options: SourceBlameOptions,
  deps: SourceBlameDeps,
): Promise<SourceBlameContributor[]> {
  ...
  const prompt = [
    `다음은 "${path.basename(sourceAbsPath)}" 파일에 대한 기여자별 커밋 목록입니다:`,
    "",
    authorSections,
    "",
...
function printBlamePretty(data: SourceBlameData): void {

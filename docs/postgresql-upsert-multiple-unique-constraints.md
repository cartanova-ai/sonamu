...
for (const uniqueIndex of table.uniqueIndexes) {
  // 조회할 조건들 추출
  const conditions = rowsWithoutId.map(row => 
    uniqueIndex.columns.map(col => row[col.name])
  );
  ...
  const existingMap = new Map(
    existingRows.map(existing => {
      const key = uniqueIndex.columns
        .map(col => existing[col.name])
        .join('|||');  // delimiter

...
export const Route = createFileRoute("/admin/documents/")({
  head: () => ({
    meta: [
      { title: "DOCUMENT List" },
      { name: "description", content: SD("entity.listManage")("DOCUMENT") },
    ],
  }),
...
function createDocumentColumns(
  ...
  return [
    ...
    {
      label: SD("entity.Document.title"),
      tc: (row) => <>{row.title}</>,
    },
    {
      label: SD("entity.Document.status"),
      tc: (row) => (
...
function DocumentList() {
  ...
  const handleConfirmDelete = () => {
    if (itemToDelete) {
      DocumentService.del([itemToDelete.id]).then(() => {
        refetch();
      });
    }
            ...
            <CardHeader className="pb-0 px-0 pt-0">
              <div className="bg-gray-100 px-6 py-4 space-y-3">
                <div className="flex items-center gap-3 flex-wrap">
                  <EnumSelect
                    enum={DocumentSearchField}
                    labels={DocumentSearchFieldLabel}
                    {...register("search")}

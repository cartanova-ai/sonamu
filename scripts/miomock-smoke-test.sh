...
cd examples/miomock/web && pnpm preview > /tmp/miomock-web-preview.log 2>&1 & PID=$!
wait_for_http $PID http://localhost:4173 /tmp/miomock-web-preview.log
kill_port 4173

echo "Testing miomock-web (dev)..."
cd examples/miomock/web && pnpm dev > /tmp/miomock-web-dev.log 2>&1 & PID=$!
wait_for_http $PID http://localhost:10281 /tmp/miomock-web-dev.log
kill_port 10281
